...
check_mole_update() {
    ...
    if is_cache_valid "$cache_file"; then
        ...
        if command -v curl > /dev/null 2>&1; then
            ...
            curl -fsSL --connect-timeout 3 --max-time 5 https://api.github.com/repos/SpreadSheets600/linux-mole/releases/latest 2> /dev/null | grep '"tag_name"' | sed -E 's/.*"v?([^"]+)".*/\1/' > "$temp_version" &
            local curl_pid=$!
